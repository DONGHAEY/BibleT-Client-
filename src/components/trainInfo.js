...
const TrainInfo = () => {
    ...
    const trackComponents = tracks.map((track, i) => {
            ...
            return (
            <div key={trackDate} style={{marginBottom:'15px', marginTop:'30px', textAlign:"center", width:"350px",height:"160px", borderRadius:"8px", backgroundColor:오늘.toLocaleDateString() === trackDate.toLocaleDateString() ? "white" : "whitesmoke"}}>
                    ...
                    <div>
                        <h1 style={{marginTop:'30px'}}>{train.trainName}</h1>
                            ...
                            {trainProfile.role === "ROLE_CAPTAIN" && <div style={{marginTop:'15px'}}>
                                <button style={{backgroundColor:'black', width:'130px', height:'30px', borderRadius:'5px', border:0, color:'white'}}>track등록하기</button>
                                </div>}
                    </div>
                    {trackComponents}
                </div>
