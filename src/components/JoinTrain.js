...
const JoinTrain = () => {
    ...
    return (
        <HeaderWithBack /> &&
         train && <div style={{display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center'}}>
            <h1>#{train.id}번</h1>
            ...
            <input style={{marginInline:'5px'}} value={nickName} onChange={e=> setNickName(e.target.value)} placeholder="보여질이름"></input>
            <button onClick={() => {
                axios.post(`/api/train/${trainId}/join`, {
                    joinKey, nickName
                }).then((response) => {
                    alert(response.data);
