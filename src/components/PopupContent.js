...
export default function PopUp({onClose, train}) {
    ...
    return (
        <Container >
            ...
            <div>
                <input style={{width:'150px', marginBlock:'15px'}} value={content} onChange={(e) => setContent(e.target.value)} placeholder="기타사항" />
            </div>
            <button style={{width:'20%', height:'8%', border:0, backgroundColor:'black', color:'white'}} onClick={() => createTrackApi()}>추가하기</button>
            <p style={{fontSize:'8px', cursor:'pointer', marginTop:'5px'}} onClick={() => onClose(false)}>취소</p>
            </div>
        </Container>
    );
}
